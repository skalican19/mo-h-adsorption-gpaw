...
`scripts/setup_perun_uma_env.sh`, `scripts/submit_perun_adsorbml.sh`, `scripts/perun_adsorbml_worker.sh`.
...
to `/projects/<proj>/hf_cache`. It should end by printing `torch.cuda.is_available() ... True`
...
```bash
cd ~/mo-h-adsorption-gpaw
STEP=1 ACCOUNT=<proj> SIF_PATH=~/containers/pytorch-ngc.sif \
  INCLUDE="Mo2N_*" \
...
| `SIF_PATH` | — (required) | container from step 1 (`~/containers/pytorch-ngc.sif`) |
...
| `ADSORBML_DATA_ROOT` | repo `data/` | set to `/scratch/<proj>` or `/projects/<proj>` for big runs; **steps 1 & 2 must match** |
| `WORKERS` | auto (1/GPU) | override worker count |

## Gotchas

- **Set the right `NGC_TAG` in step 1.** The default is a guess — pick a real
  `nvcr.io/nvidia/pytorch` aarch64 tag (`NGC_TAG=<tag> bash scripts/setup_perun_uma_env.sh ...`).
