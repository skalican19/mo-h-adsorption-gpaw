...
sbatch \
  ...
  --export=ALL,WORKFLOW_ROOT="${WORKFLOW_ROOT}",STEP="${STEP}",SIF_PATH="${SIF_PATH}",UMA_PYTHON="${UMA_PYTHON}",HF_HOME="${HF_HOME}",HF_OFFLINE="${HF_OFFLINE}",ADSORBML_DATA_ROOT="${ADSORBML_DATA_ROOT}",INCLUDE="${INCLUDE}",WORKERS="${WORKERS}" \
  "${WORKFLOW_ROOT}/scripts/perun_adsorbml_worker.sh"
